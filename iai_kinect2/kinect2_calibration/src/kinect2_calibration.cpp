...
private:
  ...
  void callback(const sensor_msgs::Image::ConstPtr imageColor, const sensor_msgs::Image::ConstPtr imageIr, const sensor_msgs::Image::ConstPtr imageDepth)
  {
    ...
    if(mode == COLOR || mode == SYNC)
    {
      ...
      if(color.empty())
      {
        ROS_INFO("color empty...");
      }
      // else{
      //   cv::imshow("color_src",color);
      // }
      //end
    }
...
int main(int argc, char **argv)
{
  ...
  // cv::Size boardDims = cv::Size(7, 6); //andyoyo 2019/1/21
  // float boardSize = 0.108;             //andyoyo 2019/1/21
  cv::Size boardDims = cv::Size(8, 11);   //add by andyoyo 2019/1/21
  float boardSize = 0.01;                 //add by andyoyo 2019/1/21
