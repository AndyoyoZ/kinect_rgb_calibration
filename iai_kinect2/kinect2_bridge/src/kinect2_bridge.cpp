...
private:
  ...
  cv::Mat video_frame_color;//RGB相机彩色图
  cv::Mat video_frame_mono; //RGB相机灰度图
...
private:
  ...
  void main()
  {
    ...
    for(; running && ros::ok();)
    {
      ...
      {
        get_video_frame = false;
        //ROS_INFO("frame is empty!!!");
      }
      else
      {
        get_video_frame = true;
        this->video_frame_color = video_frame;
        cv::cvtColor(this->video_frame_color,this->video_frame_mono,CV_BGR2GRAY);
        // cv::imshow("video_mono",this->video_frame_mono);
        // cv::waitKey(10);
      }
  ...
  void receiveColor()
  {
    ...
    if(status[CAP_MONO])
    {
      this->video_frame_mono.copyTo(images[CAP_MONO]);
    }
    if(status[CAP_BGR])
    {
      this->video_frame_color.copyTo(images[CAP_BGR]);
    }
    //end
    listenerColor->release(frames);
    lockColor.unlock();
...
int main(int argc, char **argv)
{
  ...
  if(!cap.isOpened())
  {
    ...
    ROS_INFO("Can not open video capture!!!");
  }
  else
  {
    ROS_INFO("Open video capture success!!!");
    capture_opened = true;
  }
  // end 
#if EXTENDED_OUTPUT
