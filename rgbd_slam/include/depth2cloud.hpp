...
using namespace cv;
    ...
    bool loadyamlfiles(const std::string calib_file_path);
    bool getHomographyMatrixformChessboardCorners(cv::Mat &img1,cv::Mat &img2,cv::Mat &H,int method);
    bool calc_HomographyMatrix(cv::Mat &homographyMatrix);
    bool calc_HomographyMatrix(std::string file_path,cv::Mat &homographyMatrix);
...
void runRGB2IR(cv::Mat &input_rgb,cv::Mat &input_ir,cv::Mat &input_homographyMatrix,cv::Mat &output_trans)
{
    ...
    //color camera
    cv::FileStorage fs2(calib_file_path+"/calib_color.yaml", cv::FileStorage::READ);
    if(!fs2.isOpened())
    {
    ...
    //depth shift
    cv::FileStorage fs4(calib_file_path+"/calib_depth.yaml",cv::FileStorage::READ);
    if(!fs4.isOpened())
    {
    ...
    fstream f1;
    f1.open("Homography.txt",ios::app); 
    if (f1.is_open())  
    { 
         f1 << "\n"; 
         f1 << H;
    ...
    for(int img_num=0;img_num<20;img_num++)
    {
        ...
        cv::imshow("rgb",rgb);
        //计算单应矩阵
        if(getHomographyMatrixformChessboardCorners(rgb,ir,homographyMatrix,0))
        {
    ...
    for(int img_num=0;img_num<20;img_num++)
    {
        ...
        cv::String color_img_path = file_path + "/" + ss.str() + "_sync_color.png";
        cv::String ir_img_path = file_path + "/"+ ss.str() + "_sync_ir.png";
        cv::String depth_img_path = file_path + "/" + ss.str() + "_sync_depth.png";
        // 读取../data/rgb.png和../data/depth.png，并转化为点云
        ...
        cv::imshow("rgb",rgb);
        //计算单应矩阵
        if(getHomographyMatrixformChessboardCorners(rgb,ir,homographyMatrix,1))
        {
            cv::cv2eigen(homographyMatrix,H_t_Matrix);
        }
